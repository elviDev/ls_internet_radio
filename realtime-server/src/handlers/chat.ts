...
export default function chatHandler(io: Server) {
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('join-chat', async (broadcastId: string, userInfo: any) => {
    ...
    socket.on('send-message', async (broadcastId: string, messageData: any) => {
      const room = chatRooms.get(broadcastId)
    ...
    socket.on('send-announcement', (broadcastId: string, announcementData: any) => {
      if (!announcementData.isStaff) return
      
      const announcement: Partial<ChatMessage> = {
      ...
      const room = chatRooms.get(broadcastId)
      if (room) {
        room.messages.push(announcement as ChatMessage)
      }
    ...
    socket.on('react-to-message', (broadcastId: string, messageId: string, reaction: string) => {
    ...
    socket.on('edit-message', (broadcastId: string, messageId: string, newContent: string) => {
      const room = chatRooms.get(broadcastId)
      const user = room?.users.get(socket.id)
    ...
    socket.on('delete-message', (broadcastId: string, messageId: string) => {
      const room = chatRooms.get(broadcastId)
      const user = room?.users.get(socket.id)
    ...
    socket.on('typing-start', (broadcastId: string, username: string) => {
      const room = chatRooms.get(broadcastId)
      if (!room) return
    ...
    socket.on('typing-stop', (broadcastId: string, username: string) => {
      const room = chatRooms.get(broadcastId)
      if (!room) return
